...
**ASSIGNMENT:**    
 1. Find the 'best' sales target price for predicting sales -  
  I wanted to tune my algorithm to predict a sales price that was in the upper 50 percentile of car sales prices. Since the median car sales price was 9130, I knew that the car price should be within a few thousand dollars of that price range. I experimented with different prices the 7000 to the 15000 dollar range. My experiments showed me that my best results were when i settled around 11000 as a price range for predicting sales. This is consistent with the logic that moderately priced cars tend to be in good quality, and selling them is easier than selling junk cars and high end vehicles.
  
  2. Determing values of age, gender, income level or commute distance *i.e.* miles/wk that predict best sales  
  Gender did not help predict sales so it was removed from the test set. Age was a useful variable that increased the proportion of accurate predictions by a lot. In this example i selected buyers under the age of 45 and that increased the accuracy of the model.Income was a strong predictor of the person's capacity to buy a car in the 11000 dollar range. I narrowed the focus to persons who had an income higher than 6000, which is slightly bellow the median value of income from the sample data.Debt is access to money, so it was a strong predictor of whether or not a buyer has a credit history. People who accumulate debt are likely to accumulate more debt as time goes on. I achieved good results when I experimented with the debt values. The avaerage income was about 6000 dollars. 12000 dollars is roughly equal to two years of income, and more debt than that would mean that a person isn't handling their finances well. I limited the amount of debt at 12000. Most people drive their cars between 25 and 30 miles a day or less. I narrowed the focus of the algorithm to select drivers who drive less than 25 miles a day.
  
  3. Determing the appropriate neural net - how many hidden layers, what variables to leave out?  
 I wanted to make sure that my machine could run the neural network. I decided that there should be two layers, and each with three nodes. The more simulations, the better fitted the model will be. In the prediction model I decided to leave out the miles variable, since it seemed less significant than income and debt when i experimented with changing values.
  ...
  Based on my findings the advertising campaign should focus on people younger than 45 who earn more than $6000. It's okay if they have debt, because this shows they have active lines of credit. However it should be managable so they should have less than 12000. This ad targets people who drive less than 25 miles a day, because that input made a small increase in the models prediction rates. 
  
  6.  My machine could only handle 5 bagging tests of the neural network. I first ran the knit using 10 like the class example, then I tried using 25 and recieved the same error. 5 bagging tests were processed by my computer just fine, and six gave the error again. Based on the bagging results, the five tests are pretty consistent in their good performance. All of the bagged tests successfully identified a large proportion of successful car sales. Accurate sales predictions outnumbered false sales predictions by about 20 to 1 (or 5 percent fail rate). 
